...
public class Leap2ProtoConverter {
	...
	public static Frame convert(com.leapmotion.leap.Frame frame) {
		return Frame.newBuilder()
				...
				.setFingers(convert(frame.fingers()))
				.setGestures(convert(frame.gestures()))
				.setHands(convert(frame.hands()))
				.setPointables(convert(frame.pointables()))
				.setTools(convert(frame.tools()))
	...
	protected static Gesture convert(com.leapmotion.leap.Gesture gesture) {
		return Gesture.newBuilder()
				...
				.setHands(convert(gesture.hands()))
				.setPointables(convert(gesture.pointables()))
	...
	protected static Hand convert(com.leapmotion.leap.Hand hand) {
		return Hand.newBuilder()
				...
				.setFingers(convert(hand.fingers()))
				.setPointables(convert(hand.pointables()))
	...
	protected static PointableList convert(com.leapmotion.leap.PointableList pointableList) {
		return PointableList.newBuilder()
				.setCount(pointableList.count())
				.setEmpty(pointableList.isEmpty())
				.setFrontmost(convert(pointableList.frontmost()))
				.setLeftmost(convert(pointableList.leftmost()))
				.setRightmost(convert(pointableList.rightmost()))
				.build();
	}
	...
	protected static Pointable convert(com.leapmotion.leap.Pointable pointable) {
		Pointable converted = Pointable.newBuilder()
				.setId(pointable.id())
				.setValid(pointable.isValid())
				.setDirection(convert(pointable.direction()))
				.setLength(pointable.length())
				.setWidth(pointable.width())
				.setFrame(pointable.frame().id())
				.setHand(pointable.hand().id())
				.setExtended(pointable.isExtended())
				.setFinger(pointable.isFinger())
				.setTool(pointable.isTool())
				.setStabilizedTipPosition(convert(pointable.stabilizedTipPosition()))
				.setTipPosition(convert(pointable.tipPosition()))
				.setTipVelocity(convert(pointable.tipVelocity()))
				.setTouchZone(Zone.valueOf(pointable.touchZone().ordinal()))
				.setTouchDistance(pointable.touchDistance())
				.setTimeVisible(pointable.timeVisible())
				.build();
		return converted;
	}
	...
	protected static FingerList convert(com.leapmotion.leap.FingerList fingerList) {
		FingerList.Builder builder = FingerList.newBuilder()
				.setCount(fingerList.count())
				.setEmpty(fingerList.isEmpty())
				.setFrontmost(convert(fingerList.frontmost()))
				.setLeftmost(convert(fingerList.leftmost()))
	...
	protected static Finger convert(com.leapmotion.leap.Finger finger) {
		Finger.Builder builder = Finger.newBuilder()
				.setId(finger.id())
				.setValid(finger.isValid())
				.setDirection(convert(finger.direction()))
				.setLength(finger.length())
				.setWidth(finger.width())
				.setFrame(finger.frame().id())
				.setHand(finger.hand().id())
				.setExtended(finger.isExtended())
				.setFinger(finger.isFinger())
				.setTool(finger.isTool())
				.setStabilizedTipPosition(convert(finger.stabilizedTipPosition()))
				.setTipPosition(convert(finger.tipPosition()))
				.setTipVelocity(convert(finger.tipVelocity()))
				.setTouchZone(Zone.valueOf(finger.touchZone().ordinal()))
				.setTouchDistance(finger.touchDistance())
				.setTimeVisible(finger.timeVisible())
				.setBones(createBoneList(finger))
	...
	protected static ToolList convert(com.leapmotion.leap.ToolList toolList) {
		return ToolList.newBuilder()
				.setCount(toolList.count())
				.setEmpty(toolList.isEmpty())
				.setFrontmost(convert(toolList.frontmost()))
				.setLeftmost(convert(toolList.leftmost()))
				.setRightmost(convert(toolList.rightmost()))
				.build();
	}
	...
	protected static Tool convert(com.leapmotion.leap.Tool tool) {
		return Tool.newBuilder()
				.setId(tool.id())
				.setValid(tool.isValid())
				.setDirection(convert(tool.direction()))
				.setLength(tool.length())
				.setWidth(tool.width())
				.setFrame(tool.frame().id())
				.setHand(tool.hand().id())
				.setExtended(tool.isExtended())
				.setFinger(tool.isFinger())
				.setTool(tool.isTool())
				.setStabilizedTipPosition(convert(tool.stabilizedTipPosition()))
				.setTipPosition(convert(tool.tipPosition()))
				.setTipVelocity(convert(tool.tipVelocity()))
				.setTouchZone(Zone.valueOf(tool.touchZone().ordinal()))
				.setTouchDistance(tool.touchDistance())
				.setTimeVisible(tool.timeVisible())
				.build();
	}
	...
	protected static Arm convert(com.leapmotion.leap.Arm arm) {
		return Arm.newBuilder()
				.setBasis(convert(arm.basis()))
				.setCenter(convert(arm.center()))
				.setDirection(convert(arm.direction()))
				.setElbowPosition(convert(arm.elbowPosition()))
				.setValid(arm.isValid())
				.setWidth(arm.width())
				.setWristPosition(convert(arm.wristPosition()))
				.build();
	}
	...
	protected static Bone convert(com.leapmotion.leap.Bone bone) {
		return Bone.newBuilder()
				.setBasis(convert(bone.basis()))
				.setCenter(convert(bone.center()))
				.setDirection(convert(bone.direction()))
				.setValid(bone.isValid())
				.setLength(bone.length())
				.setNextJoint(convert(bone.nextJoint()))
				.setPrevJoint(convert(bone.prevJoint()))
				.setType(com.incquerylabs.iot.leapmotion.proto.LeapMotionProtos.Bone.Type.valueOf(bone.type().ordinal()))
				.build();
	}
	...
	protected static BoneList createBoneList(com.leapmotion.leap.Finger finger) {
		...
		return BoneList.newBuilder()
				.addElement(distal)
				.addElement(intermediate)
				.addElement(proximal)
				.addElement(metacarpal)
